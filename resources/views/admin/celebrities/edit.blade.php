        ...
        <div class="col-lg-7 col-xl-7 col-md-12 col-sm-12">
            <div class="card  box-shadow-0 ">
                <div class="card-header">
                    <h4 class="card-title mb-1">Update Celebrity Info</h4>
                </div>
                <div class="card-body pt-0">
                    ...
                    @endif
                    <form method="POST" action="{{ route('celebrities.update', $celebrity->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="username" class="form-control" id="exampleInputEmail1"
                                    placeholder="Enter Email" value="{{ $celebrity->username }}">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="mb-4">
                                ...
                                <select name="status" class="form-control">
                                    @foreach (\App\Models\User::STATUS as $status)
                                        <option value="{{ $status }}"
                                        {{ (old('status') ? old('status') : $celebrity->status ?? '') == $status ? 'selected' : '' }}>
                                            @if ($status == 1)
                                                Active
                                            @else
                                                Inactive
