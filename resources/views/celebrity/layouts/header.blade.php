                        ...
                        <div class="main-header-profile bg-primary p-3">
                            <div class="d-flex wd-100p">
                                ...
                                <div class="ml-3 my-auto">
                                    <h6>{{Auth::user()->first_name . ' ' . Auth::user()->last_name}}</h6><span>Celebrity Member</span>
                                </div>
                            </div>
                        </div>
                        <a class="dropdown-item" href="{{route('profile_celebrity',\App\Models\Celebrity::find(Auth::user()->id)->id)}}"><i class="bx bx-user-circle"></i>Profile</a>
                        <a class="dropdown-item" href="{{ route('admin.logout') }}"
                            onclick="event.preventDefault();
                                                     document.getElementById('logout-form').submit();">
                            <i class="bx bx-log-out"></i> Sign Out
                        </a>
                        <form id="logout-form" action="{{ route('celebrity.logout') }}" method="POST" class="d-none">
                            @csrf
                        </form>
                    </div>
                </div>
